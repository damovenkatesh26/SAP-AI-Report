...
const Dashboard = () => {
    ...
    const fetchData = async () => {
        await axios.get('https://37.27.194.66:8095/monthly-sales-dashboard')
            .then((res) => {
                const data = res.data;
                ...
                setTopItems(data['top items sold']?.data?.slice(0, 5).map(item => ({
                    id: item.ItemCode,
                ...
                const served = data['List the customers who were served by the sales representative during the given month, and list the customers who were not served by the same representative in that month']?.data || []; // assuming your API returns this
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Grid container spacing={3}>
                {/* Sales vs Last Year */}
                <Grid item xs={12} md={12} mixWidth={100} >
                    <Paper elevation={3} sx={{ p: 2 }}>
                        ...
                        <BarChart
                            ...
                            series={[{ data: salesData.map(d => d.TotalSales), label: 'Sales', color: '#1976d2' }]}
                            width={1200}
                            height={300}
                        />
                    </Paper>
